...
<script type="text/javascript">
    ...
    function displayData(data) {
            ...
            if (data.length === 0) {
                container.append("<p>No Borrows available</p>");
            } else {
                var table = "<table border='1'><tr><th>SNo</th><th>Book Name</th><th>Borrower</th></tr>";
                for (var i = 0; i < data.length; i++) {
                    table += "<tr><td>" + num + "</td><td class='fn' onclick='showBorrows("+data[i].bookId+")'>" + data[i].bookName + "</td><td class='fn' onclick=showUserBorrows('"+ data[i].borrower+"')>" + getUser(data[i].borrower) + "</td></tr>";  
                    num++;
                }
                table += "</table>";
    ...
    function searchUsers() {
            ...
            var filtereduser = Data.filter(function (Data) {
                return (
                    Data.bookName.toLowerCase().includes(searchTerm)
                );
            });

            displayData(filtereduser);
        }
        function showBorrows(BookID) {
    var borrowInfoArray = Data.filter(function(borrow) {
        return borrow.bookId === BookID;
    });

    if (borrowInfoArray.length > 0) {
        var popupContent = "<h3>Borrow Information for Book ID: " + BookID + "</h3>"+"<h3><strong>Book Name:</strong>"+ borrowInfoArray[0].bookName +"</h3>";
        
        borrowInfoArray.forEach(function(borrowInfo) {
            popupContent +=  "<p><strong>Borrower:</strong> " + getUser(borrowInfo.borrower) + "</p>" +
                             "<hr>";
        });
        ...
        alert("Borrow information not found for Book ID: " + BookID);
    }
}
function showUserBorrows(userid)
{
    ...
    if (borrowInfoArray.length > 0) {
        ...
        alert("Borrow information this user: " + getUser(userid));
    }
}
